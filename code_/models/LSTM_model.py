...
class DLSTM: #dynamic LSTM
    ...
    def forward(self, x, S):
        """
        x: (seq_len, batch, input_dim)
        S: (seq_len,) array of dynamic window‐sizes S_t
        Returns y_seq: (seq_len, batch, output_dim)
        """
        seq_len, B, _ = x.shape
        ...
        for t in range(seq_len):
            xt = x[t]
            z = np.hstack([h, xt])  # (B, D)

            # 1) raw forget‐gate pre‐activation
            u_f = z.dot(self.Wf) + self.bf
            f0 = self.sigmoid(u_f)

            # 2) dynamic weight = S_t / S_{t-1}
            if t == 0:
                w_t = 1.0
            else:
                w_t = S[t] / float(S[t-1] + 1e-12)

            # 3) scaled forget gate
            f = f0 * w_t

            # other gates as usual
            i = self.sigmoid(z.dot(self.Wi) + self.bi)
            c_tilde = np.tanh(z.dot(self.Wc) + self.bc)
            c = f * c + i * c_tilde
            o = self.sigmoid(z.dot(self.Wo) + self.bo)
            ...
            # cache everything we need for backward
    ...
    def backward(self, x, dy):
        ...
        for t in reversed(range(seq_len)):
            ...
            # c~ gate
            dc_t = dc * i
            du_c = dc_t * (1 - c_tilde**2)

            # forget‐gate: remember c_prev is in cache[t-1][7]
            c_prev = self.cache[t-1][7] if t>0 else np.zeros_like(c)
            # gradient wrt effective forget f:
            df = dc * c_prev
            # now split gradient through scaling and through sigma
            # ∂L/∂f0 = df * w_t    (w_t treated as constant)
            du_f0 = df * w_t
            # ∂L/∂u_f = du_f0 * f0*(1-f0)
            du_f = du_f0 * f0 * (1 - f0)
